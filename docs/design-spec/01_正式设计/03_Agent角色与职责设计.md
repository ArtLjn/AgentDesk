...
| ProcessorAgent | 生成处理方案 | 内容、分类、优先级、知识库上下文 | `result`、`references` |
...
ProcessorAgent 负责生成工单解决方案。对于技术和账务类工单，它应结合分类结果、优先级和知识库检索内容给出具体处理建议。

设计上，ProcessorAgent 不负责决定是否通过审核，也不负责通知用户。这些职责分别交给 ReviewerAgent 和工作流后续节点。
