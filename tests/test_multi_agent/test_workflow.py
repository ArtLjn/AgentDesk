...
class TestTicketWorkflow:
    ...
    async def test_retry_mechanism(self):
        ...
        # 最终应完成（retry_count 递增后占位评分为 0.55，会再触发一次 retry_check，
        # retry_count 变为 3 后 review_score 重置为 None，重新审核时 base_score=0.85-3*0.15=0.4，
        # 再触发 retry_check，此时 retry_count=3 >= 3，走 handle_failure）
        # 实际行为取决于 LangGraph 状态合并逻辑
        assert result["status"] in ("completed", "failed")

    def test_max_retry_failure(self):
        """超过最大重试次数后走 handle_failure 路径。"""
        initial = create_initial_state("系统报错")
        initial["retry_count"] = 3

        decision = retry_decision(initial)

        assert decision == "handle_failure"
...
class TestRetryDecision:
    ...
    def test_at_max_retries(self):
        ...
        assert retry_decision(state) == "handle_failure"
